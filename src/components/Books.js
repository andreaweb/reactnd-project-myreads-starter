...
export class Books extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		/*controls books to be displayed depending on component/bookshelf*/
		if(nextProps !== this.props){
			if(this.props.index > -1){
				/*filters book according to shelf if Bookshelves component (with index) */
				this.setState({arr: nextProps.books.filter(
					book => book.shelf === this.props.bookshelvesValues[this.props.index]
					)
				})
